...
_tf_env = Environment(
    loader=FileSystemLoader(os.path.join(os.path.dirname(__file__), 'templates', 'tf')),
...
def _pascal(s: str) -> str:
    """Convert snake_case / hyphen-case name to PascalCase CFN logical ID."""
    return ''.join(p.capitalize() for p in re.split(r'[_\-\s]+', s) if p)


def _cfn_str(v) -> str:
    """Quote a scalar value for inline YAML if it contains special chars."""
    s = str(v)
...
def generate_cfn(data: dict) -> str:
    """Generate a single CloudFormation YAML template from the AWS payload."""
    ...
    L = []
    out = []  # (output_key, logical_id, attr_name)
...
def generate_all(data: dict) -> dict:
    """Generate all Terraform/CloudFormation files. Returns a dict of path -> content."""
...
def azure_build_root_tfvars(vnets, infras, clusters, subscription_id='', resource_group_name='', location='eastus', tags=None, iac_tool='terraform'):  # noqa: ARG001
